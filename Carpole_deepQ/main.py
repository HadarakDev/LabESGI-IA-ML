...
class DeepQLearningSolver():  # Neural Net for Deep Q-Learning - reinforcement learning algo class
    ...
    def __init__(self, ENV_NAME, observation_space, action_space):
        ...
        self.model.add(Dense(24, input_shape=(observation_space,),
                             activation="relu"))  # input shape of four due to four observations
        self.model.add(Dense(24, activation="relu"))
        self.model.add(
            Dense(self.action_space, activation="linear"))  # output two as only push cart to right or left are required
    ...
    def act(self, state):
        if np.random.rand() < self.exploration_rate:  # X% of the time NOTE: Exploration Rate changes with steps due to exploration decay
            ...
            q_values = self.model.predict(
                state)  # Predict quality value - predicts the confidence of using left or right movement of cart
            if DEBUG:
        ...
        for state, action, reward, state_next, terminal in batch:
            ...
            if not terminal:  # i.e. reward not negative
                q_update = (reward + GAMMA * np.amax(self.model.predict(state_next)[
                                                         0]))  # Gamma = discount factor & max predicted quality score for next step
            q_values = self.model.predict(
                state)  # q_values are the confidence/quality values over whether the cart needs to go left or right in next frame
            q_values[0][action] = q_update
            self.model.fit(state, q_values, verbose=0)
        self.exploration_rate *= EXPLORATION_DECAY  # Apply exploration decay
        self.exploration_rate = max(EXPLORATION_MIN,
                                    self.exploration_rate)  # Ensure doesnt go below specified minimum value
...
def poleBalance():
    ...
    while True:  # Each actual simulation run instance
        ...
        while True:  # Each timestep within said simulation...
            ...
            action = dqnObj.act(
                state)  # state will be a first iteration new state, or will be the last time steps state
            if DEBUG:
                if action == 0:
